...
def audience_sub(sub):
    """Show top represented countries"""
    if sub == True:
        return 'Subscribed'
    else:
        return 'Non Subscribed'
def pct_80(x): return np.percentile(x, 80)
def pct_20(x): return np.percentile(x, 20)
#st.cache
def load_data(): 
...
#merge daily data with publish data to get deltas
...
df_time_diff_yr = df_time_diff[df_time_diff['Video publish time'] >= date_12mo].reset_index()

# get daily view data (first 30), median & percentiles 
views_days = pd.pivot_table(df_time_diff_yr,index= 'days_published',values ='Views', aggfunc = [np.mean,np.median,pct_80, pct_20]).reset_index()


views_days.columns = ['days_published','mean_views','median_views','80pct_views','20pct_views']
views_days = views_days[views_days['days_published'].between(0,30)]
views_cumulative = views_days.loc[:,['days_published','median_views','80pct_views','20pct_views']] 
views_cumulative[['median_views','80pct_views','20pct_views']] = views_cumulative.loc[:,['median_views','80pct_views','20pct_views']].cumsum()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_date_24mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months =24)
    metric_medians6mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_6mo].median()
    metric_medians24mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo].median()
    ...
    for i in metric_medians6mo.index:
        with columns[count]:##the with aligns it to whichever column is called out
            delta = (metric_medians6mo[i] - metric_medians24mo[i])/metric_medians24mo[i]
    ...
    df_agg_numeric_lst = df_agg_diff_final.mean().index.tolist()
    #df_agg_numeric_lst = ['Views', 'Likes']
    df_to_pct = {}
    for i in df_agg_numeric_lst:
        df_to_pct[i] = '{:.1%}'.format
    
    #st.dataframe(df_agg_diff_final.style.applymap(style_negative, props='color:red;').applymap(style_positive, props='color:limegreen;').format(df_to_pct))
    st.dataframe(df_agg_diff_final.style.hide().applymap(style_positive, props='color:limegreen;').applymap(style_negative, props='color:red;').format(df_to_pct))
...
if add_sidebar == 'Individual Video Analysis':
    ...
    agg_sub_filtered['Subbed'] = agg_sub_filtered['Is Subscribed'].apply(audience_sub)
    
    
    agg_sub_filtered.sort_values('Subbed', inplace= True) 
    
    fig = px.bar(agg_sub_filtered, x='Views', y='Subbed', color='Country',orientation ='h')
